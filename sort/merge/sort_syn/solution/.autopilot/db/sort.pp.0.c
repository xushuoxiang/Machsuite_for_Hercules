...
, hercules_checkdata checkdata[8*256*1024], int* hercules_buffer_size
...
# 2 "sort.c" 2
# 19 "sort.c"
void merge(int32_t a[2048], int start, int m, int stop, hercules_checkdata checkdata[8*256*1024],int iter_m, int iter_i){
...
# 23 "sort.c"
    ...
    hercules_check_function(checkdata, 4, i, temp[i]);
...
# 32 "sort.c"
    ...
    hercules_check_function(checkdata, 4, m+1+stop-j, temp[m+1+stop-j]);
...
# 49 "sort.c"
        ...
        if(tmp_j < tmp_i) {
            a[k] = tmp_j;
# 62 "sort.c"
        hercules_check_function(checkdata, 0, k, a[k]);
...
) {_ssdm_SpecArrayDimSize(a, 2048);_ssdm_SpecArrayDimSize(checkdata, 2097152);
#pragma HLS RESOURCE variable=a core=RAM_1P_BRAM
# 82 "sort.c"

#pragma HLS TOP name=ms_mergesort
# 82 "sort.c"
...
# 99 "sort.c"
        ...
        hercules_check_function(checkdata, 15, 0, mid);


            to = i+m+m-1;


        hercules_check_function(checkdata, 16, 0, to);
