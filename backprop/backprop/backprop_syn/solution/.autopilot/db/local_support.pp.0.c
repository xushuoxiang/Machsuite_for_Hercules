...
void input_to_data(int fd, void *vdata) {
  ...
  parse_double_array(s, data->weights1, 13*64);

  s = find_section_start(p,2);
  parse_double_array(s, data->weights2, 64*64);

  s = find_section_start(p,3);
  parse_double_array(s, data->weights3, 64*3);

  s = find_section_start(p,4);
  parse_double_array(s, data->biases1, 64);

  s = find_section_start(p,5);
  parse_double_array(s, data->biases2, 64);

  s = find_section_start(p,6);
  parse_double_array(s, data->biases3, 3);

  s = find_section_start(p,7);
  parse_double_array(s, data->training_data, 163*13);

  s = find_section_start(p,8);
  parse_double_array(s, data->training_targets, 163*3);
  free(p);
}
...
void data_to_input(int fd, void *vdata) {
  ...
  write_double_array(fd, data->weights1, 13*64);

  write_section_header(fd);
  write_double_array(fd, data->weights2, 64*64);

  write_section_header(fd);
  write_double_array(fd, data->weights3, 64*3);

  write_section_header(fd);
  write_double_array(fd, data->biases1, 64);

  write_section_header(fd);
  write_double_array(fd, data->biases2, 64);

  write_section_header(fd);
  write_double_array(fd, data->biases3, 3);

  write_section_header(fd);
  write_double_array(fd, data->training_data, 163*13);

  write_section_header(fd);
  write_double_array(fd, data->training_targets, 163*3);
}
...
int check_data( void *vdata, void *vref ) {
  ...
  VITIS_LOOP_149_1: for(i=0; i<13; i++) {
    VITIS_LOOP_150_2: for(j=0; j<64; j++) {
      diff = data->weights1[i*64 + j] - ref->weights1[i*64 + j];
      has_errors |= (diff<-(1.0e-6)) || ((1.0e-6)<diff);
    }
  }
  VITIS_LOOP_155_3: for(i=0; i<64; i++) {
    VITIS_LOOP_156_4: for(j=0; j<64; j++) {
      diff = data->weights2[i*64 + j] - ref->weights2[i*64 + j];
      has_errors |= (diff<-(1.0e-6)) || ((1.0e-6)<diff);
    }
  }
  VITIS_LOOP_161_5: for(i=0; i<64; i++) {
    VITIS_LOOP_162_6: for(j=0; j<3; j++) {
      diff = data->weights3[i*3 + j] - ref->weights3[i*3 + j];
      has_errors |= (diff<-(1.0e-6)) || ((1.0e-6)<diff);
    }
  }
  VITIS_LOOP_167_7: for(i=0; i<64; i++) {
    diff = data->biases1[i] - ref->biases1[i];
    has_errors |= (diff<-(1.0e-6)) || ((1.0e-6)<diff);
  }
  VITIS_LOOP_171_8: for(i=0; i<64; i++) {
    diff = data->biases2[i] - ref->biases2[i];
    has_errors |= (diff<-(1.0e-6)) || ((1.0e-6)<diff);
  }
  VITIS_LOOP_175_9: for(i=0; i<3; i++) {
